import cv2 as cv
import numpy as np
import matplotlib.pyplot as plt


class LhsToothExtractor:
    """
    Extracts the rectangular tooth/plate region from the LEFT-HAND-SIDE (LHS) camera image.
    
    Main steps:
    1. Perspective correction (warp) to straighten the view
    2. Fixed column crop
    3. Bilateral filter → threshold → Sobel edges → erode
    4. Detect vertical edge drops (gaps between plates)
    5. Select valid plate region (prefer two points, fallback to single + offset)
    """
    
    def __init__(self):
        # -- inital Background remove make the image as 1200 x 1200 --
        self.original_image_row_start = 400
        self.original_image_row_end = 1600
        # ── Perspective correction parameters ──
        self.col_min_top    = 480
        self.col_max_top    = 730
        self.col_min_bot    = 270
        self.col_max_bot    = 520
        self.row_min        = 0
        self.row_max        = 1200
        
        self.column_crop_min = 170
        self.column_crop_max = 1140
        
        # After warp → final width
        self.warped_width = self.col_max_bot - self.col_min_bot   # ≈ 280
        
        # ── Edge detection & plate selection parameters ──
        self.thresh_value     = 100
        self.bilateral_d      = 5
        self.bilateral_sigmaColor = 25
        self.bilateral_sigmaSpace = 3
        
        self.sobel_ksize      = 5
        self.erode_kernel     = cv.getStructuringElement(cv.MORPH_RECT, (3, 3))
        
        self.drop_threshold   = -5.0          # looking for downward drops (lhs)
        self.min_plate_dist   = 60
        self.max_plate_dist   = 120
        self.default_spacing  = 110            # used in single-point fallback
        
        # We usually crop from the lowest detected point downward
        self.final_crop_top_margin = 10        # can increase if needed
        
        # This offset is used to remove potential cavilty edges that are very close to the detected point
        self.remove_cavilty_offset = 10
        
        
        
        

    def _warp_image(self, img: np.ndarray) -> np.ndarray:
        """Apply perspective transform to straighten the view"""
        if img is None or img.size == 0:
            return np.zeros((self.row_max, self.warped_width, 3), dtype=np.uint8)

        pts_src = np.float32([
            [self.col_min_top, self.row_min],     # top-left
            [self.col_max_top, self.row_min],     # top-right
            [self.col_min_bot, self.row_max],     # bottom-left
            [self.col_max_bot, self.row_max],     # bottom-right
        ])

        pts_dst = np.float32([
            [0, 0],
            [self.warped_width, 0],
            [0, self.row_max],
            [self.warped_width, self.row_max],
        ])

        matrix = cv.getPerspectiveTransform(pts_src, pts_dst)
        warped = cv.warpPerspective(img, matrix, (self.warped_width, self.row_max))
        
        # Fixed column crop after warp
        warped = warped[self.column_crop_min:self.column_crop_max,:]


        return warped
    
    
    
    

    def _find_plate_edge_points(self, warped: np.ndarray) -> list[int]:
        """Detect the main drops (gaps) in the top edge of the plates"""
        if warped is None or warped.size == 0:
            return []

        # Tight crop — we mostly care about the top part
        tight = warped[:100, :, :]

        bilateral = cv.bilateralFilter(
            tight,
            d=self.bilateral_d,
            sigmaColor=self.bilateral_sigmaColor,
            sigmaSpace=self.bilateral_sigmaSpace
        )

        gray = bilateral[:, :, 1]  # green channel often gives good contrast here
        _, thresh = cv.threshold(gray, self.thresh_value, 250, cv.THRESH_BINARY)

        # Sobel magnitude
        sobelx = cv.Sobel(thresh, cv.CV_64F, 1, 0, ksize=self.sobel_ksize)
        sobely = cv.Sobel(thresh, cv.CV_64F, 0, 1, ksize=self.sobel_ksize)
        sobel_mag = cv.magnitude(sobelx, sobely)

        # Clean a bit
        edge_clean = cv.erode(sobel_mag, self.erode_kernel)
        

        # Find the FIRST (top-most) strong edge pixel in each column
        edge_y = np.argmax(edge_clean > 0, axis=0).astype(float)

        # Mask columns with no edge
        has_edge = np.any(edge_clean > 0, axis=0)
        edge_y[~has_edge] = np.nan

        # Look for drops (negative diff → going down)
        dy = np.diff(edge_y)
        drop_locations = np.where(dy < self.drop_threshold)[0]

        return drop_locations.tolist()
    
    
    
    
    
    

    def _select_plate_region(self, drop_points: list[int], img_width: int) -> tuple[int, int]:
        """
        From detected drop points → decide left & right x coordinates of the plate.
        Prefer two real drops if distance is reasonable, else fallback to single + offset.
        """
        if len(drop_points) >= 2:
            p1, p2 = sorted(drop_points[:2])  # take first two
            dist = p2 - p1
            if self.min_plate_dist <= dist <= self.max_plate_dist:
                return p1, p2

        # Fallback: single point or invalid pair
        if len(drop_points) >= 1:
            single_x = drop_points[0]
        else:
            single_x = img_width // 2  # worst-case fallback

        dist_left = single_x
        dist_right = img_width - single_x

        if dist_right > dist_left:
            second_x = single_x + self.default_spacing
        else:
            second_x = single_x - self.default_spacing

        second_x = np.clip(second_x, 0, img_width - 1)

        x_left = min(single_x, second_x) + self.remove_cavilty_offset
        x_right = max(single_x, second_x)

        return x_left, x_right
    
    
    
    
    

    def extract(self, original_image: np.ndarray) -> np.ndarray:
        """
        Main method: input raw camera image → output cropped tooth/plate region
        
        Returns:
            cropped plate image (numpy array) or empty array if failed
        """
        if original_image is None or original_image.size == 0:
            return np.array([])
        
        original_image = original_image[:,self.original_image_row_start:self.original_image_row_end,:]
        
        # Step 1: Perspective correction + column crop
        warped = self._warp_image(original_image)

        if warped.size == 0:
            return np.array([])

        # Step 2: Detect gap locations (drops)
        drop_points = self._find_plate_edge_points(warped)

        # Step 3: Decide left-right boundaries of the plate
        x_left, x_right = self._select_plate_region(drop_points, warped.shape[1])

        # Step 4: Crop from the lowest detected point downward
        if len(drop_points) > 0:
            top_y = int(max([0] + [drop_points[0]])) + self.final_crop_top_margin
        else:
            top_y = 0

        cropped = warped[top_y:, x_left:x_right]

        if cropped.size == 0:
            # Ultimate fallback — return full warped (better than nothing)
            return warped

        return cropped


# ────────────────────────────────────────
#   Example usage
# ────────────────────────────────────────
if __name__ == "__main__":
    # Just for testing / visualization
    extractor = LhsToothExtractor()

    # Replace with your image path
    img = cv.imread("/home/dell/ZF/ZF/31.1.2026/2partb/20260131_122335/24678979/28.png")
   
    
    if img is not None:
        result = extractor.extract(img)

        print("Shape of result : ",result.shape)
        plt.imshow(result)
        plt.show()
        
    else:
        print("Could not load image.")